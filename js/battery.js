...
// STEP 1d: A new consttant to display to select image container to display the robot
const batteryImage = document.querySelector('#battery-robot');

/* Functions
-------------------------------------------------- */
/* STEP 3a: Updated updateBatteryStatus function */
function updateBatteryStatus(battery) {
    ...
    // STEP 3c: Calculate the battery level and store in a variable.
    const percentage = battery.level * 100;
    chargeLevel.textContent = percentage + "%";
    chargeMeter.value = percentage;

    // Update Robohash image
    batteryImage.src = `https://robohash.org/${percentage}`;
}
